...
GEO_INFO="N/A"
if [[ "$IPV4" != "N/A" ]]; then
  GEO_INFO=$(curl -s "https://ipapi.co/${IPV4}/json/" | jq -r '[.city // "", .region // "", .country_name // "", .org // "Unknown ISP"] | join(", ")' 2>/dev/null)
fi
...
# --------------------- Disk Benchmark ---------------------
echo "[*] Running Disk Write test (super-safe mode)..."
DISK_RESULT=$(timeout 10 dd if=/dev/zero of=/tmp/testfile bs=1M count=256 oflag=direct status=none 2>&1 || echo "timeout")
DISK_SPEED=$(echo "$DISK_RESULT" | grep -oE '[0-9.]+ MB/s' | tail -1 | awk '{print $1}')
if [ -z "$DISK_SPEED" ]; then
  echo "[!] dd test returned no data — using fio fallback..."
  DISK_SPEED=$(timeout 15 fio --name=seqwrite --rw=write --bs=1M --size=256M --ioengine=sync --time_based --runtime=10s --group_reporting 2>/dev/null | grep -oE '[0-9.]+ MB/s' | tail -1)
fi
DISK_SPEED=${DISK_SPEED:-0}
echo "Disk Write: $DISK_SPEED MB/s"

echo "[*] Running Disk Read test..."
DISK_READ=$(timeout 15 dd if=/tmp/testfile of=/dev/null bs=1M count=256 status=none 2>&1 | grep -oE '[0-9.]+ MB/s' | tail -1)
